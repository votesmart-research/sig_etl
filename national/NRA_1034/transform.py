...
def transform_split(df: pandas.DataFrame):
    ...
    split_candidate_name = df_1['candidate_name'].str.split('/', expand=True).stack().reset_index()
    split_candidate_grade = df_1['candidate_grade'].str.split('/', expand=True).stack().reset_index()
    split_candidate_office = df_1['election_location'].str.split('/', expand=True).stack().reset_index()
    
    df_2 = pandas.concat([df_1]*2).sort_index()
    df_2.reset_index(inplace=True)

    df_3 = pandas.concat([split_candidate_name[0].rename('candidate_name'),
...
def transform_name(series:pandas.Series):
    ...
    lambda_first_last = lambda x: re.sub(f'{pat_middlename}|{pat_nickname}|{pat_suffix}', '', x) if isinstance(x, str) else x
    ...
    return pandas.concat([series_firstname.rename('firstname'),
                          series_lastname.rename('lastname'),
                          df_othernames], axis=1)
...
def transform_candidate_status(series:pandas.Series):
    ...
    return pandas.concat([series_status.rename('status'), 
                          series_party.rename('party')], axis=1)
...
def transform_election_location(series:pandas.Series):
    ...
    df_district = pandas.concat([split_location[i].str.extract(pat_district, flags=re.IGNORECASE).dropna() for i in split_location.keys()[1:]])
    ...
    sub_pattern_remove = "|".join([fr'(\s?{word}\s?)' for word in words_to_remove])

    series_district.replace(sub_pattern_remove, value='', regex=True, inplace=True)

    return pandas.concat([series_office.rename('office'),
                          series_district.rename('district')], axis=1)
...
def transform_election_date(series:pandas.Series):
    ...
    series_election_date = (series_election_monthday + "," + series_election_year).apply(lambda x: verify_date(x))

    return pandas.concat([series_election_year.rename('election_year'),
                          series_election_date.rename('election_date')
                          ], 
                          axis=1)


def save_transformed(records_transformed:dict[int, dict[str, str]], file_directory:Path):

    file_directory.mkdir(exist_ok=True)

    transformed_filename = FILENAME.format(filetype='-Transformed',
                                           additional_info='',
                                           ext='csv')
    
    df = pandas.DataFrame.from_dict(records_transformed, orient='index')
    df.to_csv(file_directory / transformed_filename, index=False)


def main(records_extracted:dict[int, dict[str, str]], export_directory:Path):

    TRANSFORMED_FILES = export_directory / "TRANSFORMED_FILES"
    ...
    df_transformed = pandas.concat([df_name['firstname'].str.strip(),
    ...
    records_transformed = df_transformed.astype(str).replace('nan', '').to_dict(orient='index')

    ## Export files
    save_transformed(records_transformed, TRANSFORMED_FILES)
