...
from national._1734.ratings.extract import main as extract
    ...
    *additional_info,
    ...
    df.to_csv(
        ...
        / (
            ...
            f"{'-' if any(additional_info) else ''}{timestamp}.csv"
        ),
        index=False,
    )
...
def main():
    ...
    if not any((args.extract, args.transform, args.match)):
        ...
        records_extracted = extract(
            FILENAME + "Ratings", args.export_path, args.html_path,  args.year
        )
        save_records(
            records_extracted,
            FILENAME + "Ratings-Extract",
            args.export_path / "EXTRACT_FILES",
            str(args.year) if args.year else "",
        )
